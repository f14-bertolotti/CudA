...
cufftReal* get_kernel_in_grid(int ksize, int gsize, int states) {
    ...
    for(int i = -ksize/2; i < (ksize%2 ? ksize/2+1 : ksize/2); ++i)
        for(int j = -ksize/2; j < (ksize%2 ? ksize/2+1 : ksize/2); ++j)
            hgrid[(i>=0?i:gsize+i)*gsize+(j>=0?j:gsize+j)] = 1.0f/((ksize*ksize-1)*states);
    hgrid[0] = 0.0f;
...
int main (int argc, char *argv[]) { 
    ...
    cufftReal* kernel    = get_kernel_in_grid(KSIZE, GSIZE, STATES);
